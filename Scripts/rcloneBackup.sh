...
BACKUP_DIR="/home/net/BackupArchives"
STATE_DIR="/home/net/BackupState"
LOG_FILE="$STATE_DIR/backup.log"
KEEP_BACKUPS=50

CONFIG_PATH="/home/net/.config/rclone/rclone.conf"
...
setup_dirs() {
    ...
    # Enforce strict permissions on user SSH keys before backup
    if [[ -d "/home/net/.ssh" ]]; then
        chmod 700 /home/net/.ssh
        find /home/net/.ssh -type f -exec chmod 600 {} + 2>/dev/null || true
    fi
}
...
calculate_hash() {
    ...
    for d in "${DIRS[@]}"; do
        if [[ -d "$d" ]]; then
            # Supress find errors in case temporary files disappear during execution
